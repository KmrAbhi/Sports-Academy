
<!doctype html>
<html lang="en">
    <head>
	<meta charset="utf-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
	
	<title>KREATIVE SPORTS PROMOTER TRUST</title>
	
	<meta name="viewport" content="width=device-width">
	
	<!-- css -->
	<link rel="stylesheet" href="css/main.css">
	<link rel="stylesheet" href="css/responsive.css" media="screen and (max-width: 900px)">
	<!--header -->
</head>
<body>

    <?php include 'header.php';?>

	
	
	<!-- body-content -->
	<div class="body-content"><div class="section-inner">
		
		<!-- thirds -->
		<div class="thirds clearfix">
			
			<!-- one-third -->
			<div class="one-third mobile-collapse">
				<img src="images/1.jpg" alt="A bird on a fence" />
				<h2>Sports</h2>
				<p>KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.</p>
			</div><!--/one-third-->
			
			<!-- one-third -->
			<div class="one-third one-third-second mobile-collapse">
				<img src="images/2.jpg" alt="A bird eating" />
				<h2>Events</h2>
				<p>KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.</p>
			</div><!--/one-third-->
			
			<!-- one-third -->
			<div class="one-third one-third-last mobile-collapse">
				<img src="images/3.jpg" alt="A cat" />
				<h2>Players</h2>
				<p>KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.</p>
			</div><!--/one-third-->
			
		</div><!--/thirds-->
		
		<!-- two-columns -->
		<div class="two-columns clearfix">
			
			<!-- main -->
			<div class="main mobile-collapse">
				
				<h2>UpComing Events Advertisements</h2>
				
				<p>KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.</p>
				
				<p>KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.</p>
				
				<p>KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.</p>
				
			</div><!--/main-->
			
			<!-- side -->
			<div class="side mobile-collapse">
				
				<!-- info-box -->
				<div class="info-box-a">
					<h4>Important Announcement</h4>
					<p>KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.</p>
				</div><!--/info-box-->
				
				<!-- info-box -->
				<div class="info-box-b hide-mobile">
					<h4>Sports Fair</h4>
					<p>KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.</p>
				</div><!--/info-box-->
				
				<!-- info-box -->
				<div class="info-box-b hide-mobile">
					<h4>Annual Meet</h4>
					<p>KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.KREATIVE SPORTS PROMOTER TRUST -- COMING SOON.</p>
				</div><!--/info-box-->
				
			</div><!--/side-->
			
		</div><!--/two-columns-->
		
	</div></div><!--/body-content-->
	<?php include 'footer.php';?>
	
	
	
</body>
</html>